...
public class NotificationCenterCS : MonoBehaviour
{
	public bool debugNotifErrors = true;
	
    private static NotificationCenterCS defaultCenter;
    public static NotificationCenterCS DefaultCenter {
		get {
			if (!defaultCenter) {
				GameObject notificationObject = new GameObject ("Default Notification Center");
 
				defaultCenter = notificationObject.AddComponent<NotificationCenterCS> ();
			}
	...
	public void PostDelayedNotification (Component aSender, string aName, float delay) { PostDelayedNotification(aSender, aName, null, delay); }
	public void PostDelayedNotification (Component aSender, string aName, Hashtable aData, float delay) { PostDelayedNotification(new Notification(aSender, aName, aData),delay); }
	public void PostDelayedNotification (Notification aNotification, float delay)
	{
		StartCoroutine (DelayNotification(aNotification, delay));
	}
	private IEnumerator DelayNotification(Notification aNotification, float delay){
	...
	public void PostNotification (Notification aNotification)
	{
		...
		if (notifyList == null && debugNotifErrors) {
		...
		//notifyList.ForEach ((Component obj) => Debug.Log (obj.name));
